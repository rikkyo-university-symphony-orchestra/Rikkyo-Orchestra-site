...
- `content/concerts/` - 1公演1ファイルで管理し、開催日で最新・過去へ自動振り分けされる公演一覧
...
新着情報は `content/news/`、公演情報は `content/concerts/` を編集します。HTMLへ反映するには、プロジェクトのフォルダで次を実行します。
...
公演情報を更新するときは、Pages CMSの「公演情報」を開き、画面上部の新規作成ボタンから1公演ずつ登録します。開催日が今日以降の公演のうち最も近い1件は必ず「最新の公演情報」に表示され、その次の公演が90日以内に続く場合は2件目も表示されます。それより先の予定もCMSへ登録しておけますが、表示条件を満たすまではサイト上に出ません。開催日を過ぎた公演は「過去の演奏会」へ自動的に移ります。毎日12時23分ごろ（日本時間）にGitHub Actionsが日付を確認するため、HTMLを移し替える作業は不要です。最新公演として表示される公演には画像を登録してください。
